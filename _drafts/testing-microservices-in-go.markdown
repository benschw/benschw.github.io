...
### Suite Setup
I'm using [gocheck](gopkg.in/check.v1) which allows for things like `setup` and `teardown` functions in addition to higher level `Assert` calls. Following, I use these fixtures to boot a server with which to test against and to add/drop the location table.
...
(Karl Matthias wrote an article, [Writing Testable Code in Go](http://relistan.com/writing-testable-apps-in-go/), with a particularly good explanation for why you should write your interfaces along side the code that uses them, not the code that implements them.)
